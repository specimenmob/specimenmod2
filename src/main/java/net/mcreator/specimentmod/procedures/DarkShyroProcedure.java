...
public class DarkShyroProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		if (world.getBiome(BlockPos.containing(x, y, z)).is(new ResourceLocation("speciment_mod:biomeshyro"))) {
			if ((world.getBlockState(BlockPos.containing(x + 0, y + 0, z + 0))).getBlock() == SpecimentModModBlocks.BLOCDARKSHYRO.get()
				...
				SpecimentModMod.queueServerWork(100, () -> {
					...
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = SpecimentModModEntities.DARK_SHYRO_BOSS.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
